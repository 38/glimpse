...
void case2()
{
	GlimpseTypeDesc_t* int_desc = glimpse_typeparser_parse_type("integer{size:32,signed:true,format:hex}");
	assert(int_desc);
	glimpse_typeparser_alias(int_desc, "int32_hex");  /* aliased type does not need to free */
	GlimpseTypeDesc_t* int_desc_ = glimpse_typeparser_parse_type("int32_hex");
	assert(int_desc == int_desc_);
	GlimpseTypeDesc_t* desc = glimpse_typeparser_parse_type("vector{seperator:'#',basetype:int32_hex}");
	assert(desc);
	assert(desc->builtin_type = GLIMPSE_TYPE_BUILTIN_VECTOR);
	assert(desc->param.vector.sep == '#');
	assert(desc->param.vector.basetype);
